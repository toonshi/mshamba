...
import { mshamba_assets } from 'declarations/mshamba_assets';

const Layout = ({ children, userType }) => {
  const location = useLocation();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { profile, disconnectWallet } = useAuth();
  const [profileImageUrl, setProfileImageUrl] = useState('');

  useEffect(() => {
    // This effect now only handles fetching the profile image URL
    ...
    const fetchProfileImage = async () => {
      if (profile && profile.profilePicture) {
        try {
          ...
          console.error("Error fetching profile image for Layout:", imgError);
          setProfileImageUrl('');
        }
      } else {
        setProfileImageUrl('');
  ...
  const farmerNavItems = [
    { path: '/farmer/dashboard', icon: LucideIcons.LayoutDashboard, label: 'Farm Profile' },
    { path: '/farmer/dashboard/records', icon: LucideIcons.FileText, label: 'Setup Investment' },
    //{ path: '/farmer/dashboard/setup-investment', icon: DollarSign, label: 'Setup Investment' },
    { path: '/farmer/dashboard/valuation', icon: LucideIcons.TrendingUp, label: 'Valuation Report' },
    { path: '/farmer/dashboard/investors', icon: LucideIcons.Users, label: 'Investors List' },
    { path: '/farmer/dashboard/graphs', icon: LucideIcons.BarChart3, label: 'Analytics' },
  ];
      ...
      `}>
        ...
        <div className="p-6 border-b bg-gradient-to-r from-green-50 to-green-100">
          <div className="flex items-center justify-between">
            ...
            >
              <LucideIcons.X className="h-6 w-6 text-green-600" />
            </button>
          </div>
        </div>
        ...
        <div className="p-4 border-t bg-green-50">
          <div className="flex items-center mb-4">
            <div className="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center mr-3 flex-shrink-0 shadow-md overflow-hidden">
              {profileImageUrl ? (
            ...
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-green-800 truncate">{profile ? profile.name : 'User'}</p>
          ...
          >
            <LucideIcons.LogOut className="h-4 w-4 mr-3" />
            Logout
          </button>
        </div>
      </div>
        ...
        <header className="bg-white shadow-lg border-b-4 border-green-500 px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              >
                <LucideIcons.Menu className="h-6 w-6 text-green-600" />
              </button>
